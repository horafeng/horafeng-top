...
- 手机端首页额外启用一套专用顶部栏 + 底部栏，只作用于 `index.html`
...
- 侧栏本身仍允许在高度不足时内部滚动
- 移动端仍保持普通页面滚动
- 但首页手机端不是沿用桌面顶栏缩放方案，而是独立的“顶部栏 + 底部栏”壳层
- 首页手机端帖子弹层打开时，这套手机壳层会被隐藏，避免遮挡关闭手势和退出入口
...
- `assets/js/common.js`
  ...
  - `assets/js/index.js`
    - 首页卡片流、弹层详情、多图查看、移动端手势、左侧资料抽屉、首页手机端上下栏
- `assets/js/archive.js`
