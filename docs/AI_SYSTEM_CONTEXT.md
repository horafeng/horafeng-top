...
- 手机端首页额外启用一套专用顶部栏，只作用于 `index.html`
...
- 空状态提示
- 背景图可直接在 CSS 中替换
- 有图条目显示封面；无图条目直接渲染为纯文字卡片，不补默认封面
- 当前页内直接弹层打开帖子详情
...
- 如果首页右栏内容过长，会回退为普通随页面流动，优先保证内容完整可见
...
- 侧栏本身仍允许在高度不足时内部滚动
- 移动端仍保持普通页面滚动
- 但首页手机端不是沿用桌面顶栏缩放方案，而是独立的一条手机顶部栏
- 这条手机顶栏会在下滑浏览时隐藏、上滑时重新出现
- 首页手机端帖子弹层打开时，这条手机顶栏会被隐藏，避免遮挡关闭手势和退出入口
...
- `assets/js/common.js`
  ...
  - `assets/js/index.js`
    - 首页卡片流、弹层详情、多图查看、移动端手势、左侧资料抽屉、首页手机端顶部栏
  - `assets/js/archive.js`
    - 归档时间线渲染、搜索、标签筛选、当前页详情弹层
- `assets/js/guestbook.js`
