...
- 当启用 `body.with-site-nav` 且桌面宽度足够时，CSS 会把 `.app-shell` 固定在视口内
- 此时主滚动容器从 `window` 变成 `.flow-panel`
- 同时右栏 `#desktop-side-panel` 在这个模式下会被隐藏
- 首页因此会退化为“左栏 + 中栏”的固定框架布局

也就是说：

- 代码里有三栏结构
- 但实际桌面主视觉更接近“两栏固定壳 + 中栏滚动”
...
- 桌面端首页和留言板在 `body.with-site-nav` 下会固定整个 `.app-shell`
- `.flow-panel` 成为主滚动容器
- 左侧面板本身也允许滚动
- 移动端则回退为 `window` 滚动
...
- 顶部导航折叠、固定壳层、主滚动容器是联动的，改滚动容易把导航逻辑改坏
